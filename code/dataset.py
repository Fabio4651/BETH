...
class BETHDataset(TensorDataset):
    ...
    def __init__(self, split='train', subsample=0):
        if split == 'train':
            data = pd.read_csv("../labelled_training_data.csv", nrows=800)
        elif split == 'val':
            data = pd.read_csv("../labelled_validation_data.csv", nrows=200)
        elif split == 'test':
            data = pd.read_csv("../labelled_testing_data.csv", nrows=200)
        else:
