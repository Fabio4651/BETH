...
def validate_vae(epoch, data_loader, model, prior, device):
    if not data_loader:
        raise ValueError("data_loader is empty or None")
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(tqdm.tqdm(data_loader, leave=False)):
            if x is None or y is None:
                print(f"Skipping iteration {i} due to None values in x or y")
                continue

            x, y = x.to(device=device, non_blocking=True), y.to(device=device, non_blocking=True)
            observation, posterior, z = model(x)

            if observation is None or posterior is None or z is None:
                print(f"Skipping iteration {i} due to None values in observation, posterior, or z")
                continue

            try:
                loss = -observation.log_prob(x) + kl_divergence(z, posterior, prior)
            except Exception as e:
                print(f"Error in loss computation at iteration {i}: {e}")
                continue

            val_loss += -torch.logsumexp(-loss.view(loss.size(0), -1), dim=1).mean() - torch.log(torch.tensor(1.))
